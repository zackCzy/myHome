...
function notice(text, fun) {
	var temp = $("#point").setCenter().css({
		top:"0",
		display:"block"
	}).animate({
		opactiy : 1,height : 130
		},function() {
			setTimeout(function() {
				temp.animate({
					opactiy : 0,height : 0
...
function sendReplyCom(type){
	...
	stateAjax({
		...
		async : true,
		message : {
			'rc.content':encodeURIComponent($(that).prev().text().isEmpty()),
		...
		run:function(text){
			...
			try {
				...
				reply_content.innerHTML=$(that).prev().text();
				//回复时间
...
function sendClick(){
	...
	if(replyTextE.text().isEmpty().length<=0){
		var temp=replyTextE.css({"border":"1px solid #ff4700"}).shake(function(){
	...
	stateAjax({
		...
		async : true,
		message : {
			'c.content':encodeURIComponent(replyTextE.text().isEmpty()),
			'id':$(that).attr("alt")
		},
		run:function(text){
			that.disabled=false;
			...
			try {
				...
				divo.innerHTML=replyTextE.text().isEmpty();
				...
				var count=$(that.parentNode).prev().find("b").eq(0);
	...
	stateAjax({
		...
		run:function(text){
			if(text=="remove is ok"){
				that.disabled=false;
				var temp=$(that.parentNode.parentNode).slideUp(300,function(){
					var count=$(this.parentNode.parentNode.parentNode.parentNode).prev().find("b").eq(0);
					count.html(parseInt(count.html())-1);
					temp.remove();
				});
			}else{
...
function sendSmallSpeak(that){
	...
	stateAjax({
		...
		message : {
			...
			'userlog.noHtmlLog':encodeURIComponent(contentText),
